...
NS_LOG_COMPONENT_DEFINE ("YoungdsrErrorBuffer");

namespace youngdsr {

uint32_t
YoungdsrErrorBuffer::GetSize ()
{
  Purge ();
...
bool
YoungdsrErrorBuffer::Enqueue (YoungdsrErrorBuffEntry & entry)
{
  Purge ();
  for (std::vector<YoungdsrErrorBuffEntry>::const_iterator i = m_errorBuffer.begin (); i
       != m_errorBuffer.end (); ++i)
    {
...
void
YoungdsrErrorBuffer::DropPacketForErrLink (Ipv4Address source, Ipv4Address nextHop)
{
   ...
   */
  for (std::vector<YoungdsrErrorBuffEntry>::iterator i = m_errorBuffer.begin (); i
       != m_errorBuffer.end (); ++i)
    {
      if (LinkEqual (*i, link))
        {
  ...
  m_errorBuffer.erase (std::remove_if (m_errorBuffer.begin (), m_errorBuffer.end (),
                                       std::bind2nd (std::ptr_fun (YoungdsrErrorBuffer::LinkEqual), link)), m_errorBuffer.end ());
}

bool
YoungdsrErrorBuffer::Dequeue (Ipv4Address dst, YoungdsrErrorBuffEntry & entry)
{
  Purge ();
  /*
   * Dequeue the entry with destination address dst
   */
  for (std::vector<YoungdsrErrorBuffEntry>::iterator i = m_errorBuffer.begin (); i != m_errorBuffer.end (); ++i)
    {
...
bool
YoungdsrErrorBuffer::Find (Ipv4Address dst)
{
  /*
...
struct IsExpired
{
   ...
   */
  bool
  operator() (YoungdsrErrorBuffEntry const & e) const
  {
...
void
YoungdsrErrorBuffer::Drop (YoungdsrErrorBuffEntry en, std::string reason)
{
...
void
YoungdsrErrorBuffer::DropLink (YoungdsrErrorBuffEntry en, std::string reason)
{
