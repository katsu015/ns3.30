...
/**
 * \class YoungdsrLinkStab
 * \brief YoungdsrLinkStab class (DSR link stability)
 */
class YoungdsrLinkStab
{
public:
   ...
   */
  YoungdsrLinkStab (Time linkStab = Simulator::Now ());
  /**
   * \brief Destructor
   */
  virtual ~YoungdsrLinkStab ();
...
/**
 * \class YoungdsrNodeStab
 * \brief YoungdsrNodeStab class (DSR node stability)
 */
class YoungdsrNodeStab
{
public:
   ...
   */
  YoungdsrNodeStab (Time nodeStab = Simulator::Now ());
  virtual ~YoungdsrNodeStab ();
...
/**
 * \class YoungdsrRouteCacheEntry
 * \brief YoungdsrRouteCacheEntry class for entries in the route cache
 */
class YoungdsrRouteCacheEntry
{
public:
   ...
   */
  YoungdsrRouteCacheEntry (IP_VECTOR const  & ip = IP_VECTOR (), Ipv4Address dst = Ipv4Address (), Time exp = Simulator::Now ());
  virtual ~YoungdsrRouteCacheEntry ();
   ...
   */
  bool operator== (YoungdsrRouteCacheEntry const & o) const
  {
 ...
 */
class YoungdsrRouteCache : public Object
{
public:
  /**
  ...
  YoungdsrRouteCache ();
  virtual ~YoungdsrRouteCache ();

  /**
   * \brief Remove the aged route cache entries when the route cache is full
   * \param rtVector the route cache to scan.
   */
  void RemoveLastEntry (std::list<YoungdsrRouteCacheEntry> & rtVector);
  /**
   * \brief Define the vector of route entries.
   */
  typedef std::list<YoungdsrRouteCacheEntry::IP_VECTOR> routeVector;
   ...
   */
  void PrintRouteVector (std::list<YoungdsrRouteCacheEntry> route);
  /**
   ...
   * \return true if same
   */
  bool FindSameRoute (YoungdsrRouteCacheEntry & rt, std::list<YoungdsrRouteCacheEntry> & rtVector);
  /**
...
private:
   ...
   * \return
   */
  YoungdsrRouteCache & operator= (YoungdsrRouteCache const &);
  YoungdsrRouteCacheEntry::IP_VECTOR m_vector;               ///< The route vector to save the ip addresses for intermediate nodes.
  uint32_t m_maxCacheLen;                               ///< The maximum number of packets that we allow a routing protocol to buffer.
  Time     RouteCacheTimeout;                           ///< The maximum period of time that youngdsr is allowed to for an unused route.
   ...
   */
  typedef std::list<YoungdsrRouteCacheEntry> routeEntryVector;
  ...
  std::map<Ipv4Address, YoungdsrRouteCacheEntry::IP_VECTOR> m_bestRoutesTable_link;     ///< for link route cache
  std::map<Link, YoungdsrLinkStab> m_linkCache;                                         ///< The data structure to store link info
  std::map<Ipv4Address, YoungdsrNodeStab> m_nodeCache;                                  ///< The data structure to store node info
  /**
   ...
   */
  bool LookupRoute_Link (Ipv4Address id, YoungdsrRouteCacheEntry & rt);
  /**
...
public:
   ...
   * \return true if the link is cached
   */
  bool AddRoute_Link (YoungdsrRouteCacheEntry::IP_VECTOR nodelist, Ipv4Address node);
  /**
