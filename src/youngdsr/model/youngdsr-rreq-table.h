 ...
 */
class YoungdsrReceivedRreqEntry
{
public:
  /**
   * Construct a YoungdsrReceivedRreqEntry with the given parameters
   *
   * \param d IPv4 address of the destination
   * \param i identification
   */
  YoungdsrReceivedRreqEntry (Ipv4Address d = Ipv4Address (), uint16_t i = 0)
    : m_destination (d),
      m_identification (i)
  {
  }
   ...
   * \param o another YoungdsrReceivedRreqEntry
   * \return true if equal
   */
  bool operator== (YoungdsrReceivedRreqEntry const & o) const
  {
...
/**
 * \ingroup youngdsr
 * \brief maintain list of YoungdsrRreqTable entry
 */
class YoungdsrRreqTable  : public Object
{
public:
  /**
...
private:
  ...
  /// Map of entries
  std::list<YoungdsrReceivedRreqEntry> m_sourceRequests;
  ...
  std::map<Ipv4Address, std::list<YoungdsrReceivedRreqEntry> > m_sourceRreqMap;
