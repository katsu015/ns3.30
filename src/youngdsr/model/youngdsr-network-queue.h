 ...
 */
class YoungdsrNetworkQueueEntry
{
public:
  /**
   * Construct a YoungdsrNetworkQueueEntry with the given parameters
   *
   * \param pa packet
   ...
   * \param r Route
   */
  YoungdsrNetworkQueueEntry (Ptr<const Packet> pa = 0, Ipv4Address s = Ipv4Address (), Ipv4Address n = Ipv4Address (),
   ...
   */
  bool operator== (YoungdsrNetworkQueueEntry const & o) const
  {
...
class YoungdsrNetworkQueue : public Object
{
public:
  /**
  ...
  YoungdsrNetworkQueue ();
  /**
   * Construct a YoungdsrNetworkQueue with the given
   * maximum length and maximum delay.
   *
   * \param maxLen Maximum queue size
   * \param maxDelay Maximum entry lifetime in the queue
   */
  YoungdsrNetworkQueue (uint32_t maxLen, Time maxDelay);
  ~YoungdsrNetworkQueue ();
   ...
   */
  bool FindPacketWithNexthop (Ipv4Address nextHop, YoungdsrNetworkQueueEntry & entry);
  /**
   ...
   *         false otherwise
   */
  bool Enqueue (YoungdsrNetworkQueueEntry & entry);
  /**
   ...
   *         false otherwise
   */
  bool Dequeue (YoungdsrNetworkQueueEntry & entry);
  /**
   * Number of entries
   *
   ...
   * \return the current queue entry
   */
  std::vector<YoungdsrNetworkQueueEntry> & GetQueue ()
  {
    return m_youngdsrNetworkQueue;
  }
...
private:
   ...
   */
  void Cleanup (void);
  std::vector<YoungdsrNetworkQueueEntry> m_youngdsrNetworkQueue; //!< Queue (vector) of entries
