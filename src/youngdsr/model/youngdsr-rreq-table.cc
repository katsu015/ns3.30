...
NS_LOG_COMPONENT_DEFINE ("YoungdsrRreqTable");
  
namespace youngdsr {

NS_OBJECT_ENSURE_REGISTERED (YoungdsrRreqTable);

TypeId YoungdsrRreqTable::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::youngdsr::YoungdsrRreqTable")
    .SetParent<Object> ()
    .SetGroupName ("Youngdsr")
    .AddConstructor<YoungdsrRreqTable> ()
  ;
  return tid;
}

YoungdsrRreqTable::YoungdsrRreqTable ()
...
void
YoungdsrRreqTable::RemoveLeastExpire ()
{
...
void
YoungdsrRreqTable::FindAndUpdate (Ipv4Address dst)
{
...
void
YoungdsrRreqTable::RemoveRreqEntry (Ipv4Address dst)
{
  NS_LOG_FUNCTION (this << dst);
  std::map<Ipv4Address, RreqTableEntry >::const_iterator i =
    m_rreqDstMap.find (dst);
...
uint32_t
YoungdsrRreqTable::GetRreqCnt (Ipv4Address dst)
{
  NS_LOG_FUNCTION (this << dst);
  std::map<Ipv4Address, RreqTableEntry >::const_iterator i =
    m_rreqDstMap.find (dst);
 ...
 */
uint32_t
YoungdsrRreqTable::CheckUniqueRreqId (Ipv4Address dst)
{
...
BlackList*
YoungdsrRreqTable::FindUnidirectional (Ipv4Address neighbor)
{
...
bool
YoungdsrRreqTable::MarkLinkAsUnidirectional (Ipv4Address neighbor, Time blacklistTimeout)
{
...
void
YoungdsrRreqTable::PurgeNeighbor ()
{
  /*
...
bool
YoungdsrRreqTable::FindSourceEntry (Ipv4Address src, Ipv4Address dst, uint16_t id)
{
  NS_LOG_FUNCTION (this << src << dst << id);
  YoungdsrReceivedRreqEntry rreqEntry;
  rreqEntry.SetDestination (dst);
  rreqEntry.SetIdentification (id);
  std::list<YoungdsrReceivedRreqEntry> receivedRreqEntryList;
  /*
   * this function will return false if the entry is not found, true if duplicate entry find
   */
  std::map<Ipv4Address, std::list<YoungdsrReceivedRreqEntry> >::const_iterator i = m_sourceRreqMap.find (src);
  if (i == m_sourceRreqMap.end ())
  ...
  else
    {
      ...
      for (std::list<YoungdsrReceivedRreqEntry>::const_iterator j = receivedRreqEntryList.begin (); j != receivedRreqEntryList.end (); ++j)
        {
