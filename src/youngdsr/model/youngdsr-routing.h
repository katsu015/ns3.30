...
/**
 * \class YoungdsrRouting
 * \brief Youngdsr Routing base
 */
class YoungdsrRouting : public IpL4Protocol
{
public:
  /**
  ...
  /**
    * \brief Define the youngdsr protocol number.
    */
   ...
   */
  void SetRouteCache (Ptr<youngdsr::YoungdsrRouteCache> r);
  /**
   * \brief Get the route cache.
   * \return the route cache
   */
  Ptr<youngdsr::YoungdsrRouteCache> GetRouteCache () const;
  /**
   * \brief Set the node.
   * \param r the request table to set
   */
  void SetRequestTable (Ptr<youngdsr::YoungdsrRreqTable> r);
  /**
    * \brief Get the request table.
    * \return the request table
    */
  Ptr<youngdsr::YoungdsrRreqTable> GetRequestTable () const;
  /**
   * \brief Set the node.
   * \param r the passive buffer to set
   */
  void SetPassiveBuffer (Ptr<youngdsr::YoungdsrPassiveBuffer> r);
  /**
    * \brief Get the passive buffer
    * \return the passive buffer
    */
  Ptr<youngdsr::YoungdsrPassiveBuffer> GetPassiveBuffer () const;

  /// functions used to direct to route cache
  //\{
  bool IsLinkCache ();
  void UseExtends (YoungdsrRouteCacheEntry::IP_VECTOR rt);
  bool LookupRoute (Ipv4Address id, YoungdsrRouteCacheEntry & rt);
  bool AddRoute_Link (YoungdsrRouteCacheEntry::IP_VECTOR nodelist, Ipv4Address source);
  bool AddRoute (YoungdsrRouteCacheEntry & rt);
  ...
  /**
    * \brief Get the youngdsr protocol number.
   ...
   */
  uint32_t GetPriority (YoungdsrMessageType messageType);
  /**
   ...
   */
  void ForwardErrPacket (YoungdsrOptionRerrUnreachHeader &rerr,
   ...
   */
  bool SendRealDown (YoungdsrNetworkQueueEntry & newEntry);
  /**
   ...
   * \param protocol number
   */
  void SendPacketFromBuffer (YoungdsrOptionSRHeader const &sourceRoute,
                             Ipv4Address nextHop,
                             uint8_t protocol);
  /**
  ...
  */
  void CancelPacketAllTimer (YoungdsrMaintainBuffEntry & mb);
  /**
   ...
   */
  void CancelNetworkPacketTimer (YoungdsrMaintainBuffEntry & mb);
  /**
   * \brief Cancel the passive packet retransmission timer for a specific maintenance entry
   * \param mb maintian byffer entry
   */
  void CancelPassivePacketTimer (YoungdsrMaintainBuffEntry & mb);
  /**
   * \brief Cancel the link packet retransmission timer for a specific maintenance entry
   * \param mb maintian byffer entry
   */
  void CancelLinkPacketTimer (YoungdsrMaintainBuffEntry & mb);
  /**
   ...
   */
  void ScheduleLinkPacketRetry   (YoungdsrMaintainBuffEntry & mb,
                                  uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void SchedulePassivePacketRetry   (YoungdsrMaintainBuffEntry & mb,
                                     uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void ScheduleNetworkPacketRetry   (YoungdsrMaintainBuffEntry & mb,
                                     bool isFirst,
                                     uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void LinkScheduleTimerExpire  (YoungdsrMaintainBuffEntry & mb,
                                 uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void NetworkScheduleTimerExpire  (YoungdsrMaintainBuffEntry & mb,
                                    uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void PassiveScheduleTimerExpire  (YoungdsrMaintainBuffEntry & mb,
                                    uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void SendErrorRequest (YoungdsrOptionRerrUnreachHeader &rerr, uint8_t protocol);
  /**
  ...
  /**
   * \brief Insert a new Youngdsr Option.
   * \param option the option to insert
   */
  void Insert (Ptr<youngdsr::YoungdsrOptions> option);
  /**
   * \brief Get the option corresponding to optionNumber.
   * \param optionNumber the option number of the option to retrieve
   * \return a matching Youngdsr option
   */
  Ptr<youngdsr::YoungdsrOptions> GetOption (int optionNumber);
  /**
 ...
 *          */
  ...
  TracedCallback <const YoungdsrOptionSRHeader &> m_txPacketTrace; ///< packet trace callback
...
private:
   ...
   * \param protocol The protocol we receive, need to verify it is youngdsr protocol
   * \param from The from address we received the packet
   * \param to The address this packet is destined for
   * \param packetType The youngdsr packet type, 0 is for control packet, 1 for data packet
   ...
   */
  typedef std::list<Ptr<YoungdsrOptions> > YoungdsrOptionList_t;
  /**
   * \brief List of DSR Options supported.
   */
  YoungdsrOptionList_t m_options;
  ...
  YoungdsrSendBuffer m_sendBuffer;                           ///< The send buffer

  YoungdsrErrorBuffer m_errorBuffer;                         ///< The error buffer to save the error messages
  ...
  YoungdsrMaintainBuffer m_maintainBuffer;                   ///< The declaration of maintain buffer
  ...
  Ptr<youngdsr::YoungdsrRouteCache> m_routeCache;                 ///< A "drop-front" queue used by the routing layer to cache routes found.

  Ptr<youngdsr::YoungdsrRreqTable> m_rreqTable;                   ///< A "drop-front" queue used by the routing layer to cache route request sent.

  Ptr<youngdsr::YoungdsrPassiveBuffer> m_passiveBuffer;           ///< A "drop-front" queue used by the routing layer to cache route request sent.
  ...
  std::map<uint32_t, Ptr<youngdsr::YoungdsrNetworkQueue> > m_priorityQueue;   ///< priority queues

  YoungdsrGraReply m_graReply;                               ///< The gratuitous route reply.

  YoungdsrNetworkQueue m_networkQueue;                       ///< The network queue.
