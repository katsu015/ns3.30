...
namespace youngdsr {
/**
 * \class YoungdsrHeader
 * \brief Header for Youngdsr Routing.
 */

/**
* \ingroup youngdsr
* \brief Youngdsr fixed size header Format
  \verbatim
   |      0        |      1        |      2        |      3        |
...
/**
* \ingroup youngdsr
* \brief The modified version of Youngdsr fixed size header Format
  \verbatim
   |      0        |      1        |      2        |      3        |
   0 1 2 3 4 5 6 7 0 1 2 3 4 5 6 7 0 1 2 3 4 5 6 7 0 1 2 3 4 5 6 7
   +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  ...
  \endverbatim
*/
class YoungdsrFsHeader : public Header
{
public:
  /**
...
/**
 * \class YoungdsrOptionField
 * \brief Option field for an YoungdsrFsHeader
 * Enables adding options to an YoungdsrFsHeader
 *
 * Implementor's note: Make sure to add the result of
 * OptionField::GetSerializedSize () to your YoungdsrFsHeader::GetSerializedSize ()
 * return value. Call OptionField::Serialize and OptionField::Deserialize at the
 * end of your corresponding YoungdsrFsHeader methods.
 */
class YoungdsrOptionField
{
public:
   ...
   */
  YoungdsrOptionField (uint32_t optionsOffset);
  /**
   * \brief Destructor.
   */
  ~YoungdsrOptionField ();
   ...
   */
  void AddYoungdsrOption (YoungdsrOptionHeader const& option);
  /**
   ...
   */
  uint32_t GetYoungdsrOptionsOffset ();
  /**
   * \brief Get the buffer.
   * \return buffer
   */
  Buffer GetYoungdsrOptionBuffer ();
...
private:
   ...
   */
  uint32_t CalculatePad (YoungdsrOptionHeader::Alignment alignment) const;
  /**
   * \brief Data payload.
   */
  Buffer m_optionData;
...
/**
 * \class YoungdsrRoutingHeader
 * \brief Header of Youngdsr Routing
 */
class YoungdsrRoutingHeader : public YoungdsrFsHeader,
                         public YoungdsrOptionField
{
public:
  /**
   * \brief Get the type identificator.
   ...
   * \brief Constructor.
   */
  YoungdsrRoutingHeader ();
  /**
   * \brief Destructor.
   */
  virtual ~YoungdsrRoutingHeader ();
  /**
   * \brief Print some information about the packet.
   * \param os output stream
   * \return info about this packet
...
static inline std::ostream & operator<< (std::ostream& os, const YoungdsrRoutingHeader & youngdsr)
{
  youngdsr.Print (os);
  return os;
}
