...
NS_LOG_COMPONENT_DEFINE ("YoungdsrSendBuffer");

namespace youngdsr {

uint32_t
YoungdsrSendBuffer::GetSize ()
{
  Purge ();
...
bool
YoungdsrSendBuffer::Enqueue (YoungdsrSendBuffEntry & entry)
{
  Purge ();
  for (std::vector<YoungdsrSendBuffEntry>::const_iterator i = m_sendBuffer.begin (); i
       != m_sendBuffer.end (); ++i)
    {
...
void
YoungdsrSendBuffer::DropPacketWithDst (Ipv4Address dst)
{
   ...
   */
  for (std::vector<YoungdsrSendBuffEntry>::iterator i = m_sendBuffer.begin (); i
       != m_sendBuffer.end (); ++i)
    {
      if (IsEqual (*i, dst))
        {
  ...
  m_sendBuffer.erase (std::remove_if (m_sendBuffer.begin (), m_sendBuffer.end (),
                                      std::bind2nd (std::ptr_fun (YoungdsrSendBuffer::IsEqual), dst)), m_sendBuffer.end ());
}

bool
YoungdsrSendBuffer::Dequeue (Ipv4Address dst, YoungdsrSendBuffEntry & entry)
{
  Purge ();
  /*
   * Dequeue the entry with destination address dst
   */
  for (std::vector<YoungdsrSendBuffEntry>::iterator i = m_sendBuffer.begin (); i != m_sendBuffer.end (); ++i)
    {
...
bool
YoungdsrSendBuffer::Find (Ipv4Address dst)
{
  /*
...
struct IsExpired
{
   ...
   */
  bool
  operator() (YoungdsrSendBuffEntry const & e) const
  {
...
void
YoungdsrSendBuffer::Drop (YoungdsrSendBuffEntry en, std::string reason)
{
