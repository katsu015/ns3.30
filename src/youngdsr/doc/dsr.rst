...
a packet buffer in youngdsr-rsendbuff.cc. The packet queue implements 
...
* The YoungdsrFsHeader has added 3 fields: message type, source id, destination id, and these changes only for post-processing
...
* DSR works as a shim header between transport and network protocol, it needs its own forwarding mechanism, we are changing the packet transmission to hop-by-hop delivery, so we added two fields in youngdsr fixed header to notify packet delivery
...
To have a node run DSR, the easiest way would be to use the YoungdsrHelper
and YoungdsrMainHelpers in your simulation script. For instance:
...
The example scripts inside ``src/youngdsr/examples/`` demonstrate the use of DSR based nodes in different scenarios. 
The helper source can be found inside ``src/youngdsr/helper/youngdsr-main-helper.{h,cc}`` 
and ``src/youngdsr/helper/youngdsr-helper.{h,cc}``

Examples
********

The example can be found in ``src/youngdsr/examples/``:

* youngdsr.cc use DSR as routing protocol within a traditional MANETs environment[3].
...
* Unit tests have been written to verify the internals of DSR. This can be found in ``src/youngdsr/test/youngdsr-test-suite.cc``. These tests verify whether the methods inside DSR module which deal with packet buffer, headers work correctly.
...
The model is not fully compliant with :rfc:`4728`. As an example, Youngdsr fixed size header
...
[1] Original paper: http://www.monarch.cs.rice.edu/monarch-papers/youngdsr-chapter00.pdf
