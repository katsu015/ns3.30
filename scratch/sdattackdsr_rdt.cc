...
#include "ns3/sdattackdsr-module.h"
#include <sstream>
...
NS_LOG_COMPONENT_DEFINE ("sdattackdsrTest");

int
main (int argc, char *argv[])
{
      std::string animFile = "sdattackdsranim.xml";
  //
...
#if 0
  LogComponentEnable ("YoungsdattackdsrOptions", LOG_LEVEL_ALL);
  LogComponentEnable ("YoungsdattackdsrHelper", LOG_LEVEL_ALL);
  LogComponentEnable ("YoungsdattackdsrRouting", LOG_LEVEL_ALL);
  LogComponentEnable ("YoungsdattackdsrOptionHeader", LOG_LEVEL_ALL);
  LogComponentEnable ("YoungsdattackdsrFsHeader", LOG_LEVEL_ALL);
  LogComponentEnable ("YoungsdattackdsrGraReplyTable", LOG_LEVEL_ALL);
  LogComponentEnable ("YoungsdattackdsrSendBuffer", LOG_LEVEL_ALL);
  LogComponentEnable ("YoungsdattackdsrRouteCache", LOG_LEVEL_ALL);
  LogComponentEnable ("YoungsdattackdsrMaintainBuffer", LOG_LEVEL_ALL);
  LogComponentEnable ("YoungsdattackdsrRreqTable", LOG_LEVEL_ALL);
  LogComponentEnable ("YoungsdattackdsrErrorBuffer", LOG_LEVEL_ALL);
  LogComponentEnable ("YoungsdattackdsrNetworkQueue", LOG_LEVEL_ALL);
#endif
  ...
  Ptr<OutputStreamWrapper> stream = ascii.CreateFileStream ("sdattackdsrtest.tr");
  ...
  */
  ...
  SdattackdsrMainHelper sdattackdsrMain;
  SdattackdsrHelper sdattackdsr;
  internet.Install (adhocNodes);
  sdattackdsrMain.Install (sdattackdsr, adhocNodes);
  ...
  wifiPhy.EnablePcapAll("sdattackdsr_rdtp");
  Simulator::Run ();
  Simulator::Destroy ();
}
